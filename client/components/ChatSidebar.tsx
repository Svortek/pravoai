import { Plus, Settings, LogOut, Trash2, X, Check, ChevronLeft, Menu } from "lucide-react";
...
export const ChatSidebar = ({
  ...
  return (
    <>
      {isCollapsed ? (
        // В свернутом состоянии - только плавающая кнопка бургера
        <button
          onClick={onToggleCollapse}
          className="fixed left-4 top-4 z-10 p-2 text-muted-foreground hover:text-foreground transition-colors"
          title="Раскрыть панель"
        >
          <Menu className="w-5 h-5" />
        </button>
      ) : (
        // В развернутом состоянии - полноценный сайдбар
